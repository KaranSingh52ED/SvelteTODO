...
export function validatePost(data: any): data is Post {
	...
	const categoryIsString = typeof data.category === 'string';
	const userIsValid = validateUser(data.user);
	const tags = data.tags;
	const tagsIsValid =
		Array.isArray(tags) &&
		tags.length > 0 &&
		tags.reduce<boolean>((prev, cur) => (prev ? typeof cur === 'string' : prev), true);
	const likes = data.likes;
	const likesIsValid =
		Array.isArray(likes) &&
		likes.reduce<boolean>((prev, cur) => (prev ? typeof cur === 'string' : prev), true);
